...
class ThetaStarPathPlanner:
    ...
    def map_callback(self, msg):
        ...
        data_array = np.array(msg.data).reshape((height, width))
        data_array = np.flipud(data_array)
        # Tạo ảnh từ mảng dữ liệu
        img = np.zeros((height, width, 3), dtype=np.uint8)
        img[data_array == 100] = [0, 0, 0]  # Tường: màu đen
        img[data_array == 0] = [255, 255, 255]  # Trống: màu trắng

        # Chuyển ảnh sang dạng Pillow để sử dụng với Turtle
        pil_image = Image.fromarray(img)
        pil_image.save("map_image.png")
    ...
    def generate_valid_map(self):
        ...
        w = int(self.size[0]/self.resolution)
        h = int(self.size[1]/self.resolution)
